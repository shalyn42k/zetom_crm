...
### БАГ-1 — Departments редактируются без `edit_users`

| | |
|---|---|
| **Кто** | Alan (Specialist) |
| **Где** | `/admin/auth/user/<id>/change/` → вкладка Departments |
| **Проблема** | Кнопки add / remove / promote / demote сохраняют изменения даже без пермишена `edit_users` |
| **Ожидается** | Кнопки заблокированы без `edit_users` |
